const ball = document.getElementById('imageContainer');
...
function playButtonSound() {
    // 播放音效
    buttonSound.currentTime = 0; // 重置音效到開始
    buttonSound.play();
}
...
function changeButtonSizeRandomly() {
    ...
    button.style.fontSize = parseInt(newHeight) * 0.5 + 'px'; // 字體大小是按鈕高度的一半
}

function updateClickCount() {
    clickCount += 1;
    document.getElementById('clickCount').innerText = clickCount;
    document.getElementById('clickCountPlaces').innerText = clickCount;
}

function update() {
    speed += gravity; // Increase speed by gravity
    position += speed; // Increase position by speed

    // If ball hits the bottom of the screen
    if (position + ball.clientHeight > window.innerHeight) {
        position = window.innerHeight - ball.clientHeight; // Set ball position to bottom
        speed *= -1.0; // Reverse speed and reduce it (simulate bounce)
    }
